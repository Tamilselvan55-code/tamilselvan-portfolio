...
export function ContactSection() {
  ...
  useEffect(() => {
    ...
    if (reduced) {
      gsap.set(
        [labelRef.current, titleRef.current, subtitleRef.current, bodyRef.current, listRef.current, formRef.current].filter(Boolean),
    ...
    >
      {SHOW_CONTACT_SECTION && (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 lg:gap-20">
          ...
          {/* Right — form or let's connect visual */}
          <div ref={formRef} className="relative" style={{ opacity: 0 }}>
            {SHOW_CONTACT_FORM ? (
              ...
              <div className="flex flex-col items-center justify-center min-h-[350px] h-full w-full relative z-10 py-10 lg:py-0">
                {/* Decorative subtle orbit */}
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[260px] h-[260px] sm:w-[300px] sm:h-[300px] border border-accent/5 rounded-full animate-[spin_40s_linear_infinite] pointer-events-none" />
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[200px] h-[200px] sm:w-[240px] sm:h-[240px] border border-border/10 rounded-full animate-[spin_30s_linear_infinite_reverse] pointer-events-none" />
                
                {/* Center Element */}
                <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-full border border-ink-primary/10 flex items-center justify-center mb-10 relative z-10 bg-canvas-base">
                  <span className="font-cinematic text-lg sm:text-xl text-ink-primary">TS</span>
                </div>
                
                {/* Heading */}
                <h3 className="text-[0.6875rem] font-sans font-medium tracking-[0.3em] uppercase text-accent mb-2 relative z-10 text-center">
                  Let's Connect
                </h3>
                <p className="text-[0.6875rem] font-sans tracking-[0.1em] text-ink-muted/70 uppercase mb-12 relative z-10 text-center">
                  Open to opportunities
                </p>

                {/* Links Grid */}
                <div className="grid grid-cols-2 gap-x-12 gap-y-8 relative z-10 text-center w-full max-w-[280px]">
                  {CONTACT_ITEMS.map(({ label, href }) => (
                    <a
                      ...
                      target={href.startsWith('http') ? '_blank' : undefined}
                      rel={href.startsWith('http') ? 'noopener noreferrer' : undefined}
                      className="text-[0.8125rem] sm:text-[0.875rem] font-sans font-medium tracking-[0.15em] uppercase text-ink-primary hover:text-accent transition-all duration-300 relative group inline-block py-1 hover:-translate-y-0.5"
                    >
                      {label}
                      <span className="absolute bottom-0 left-1/2 -translate-x-1/2 w-0 h-px bg-accent/50 transition-all duration-300 group-hover:w-full" />
                    </a>
                  ))}
                </div>
              </div>
